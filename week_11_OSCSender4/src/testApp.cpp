...
//--------------------------------------------------------------
void testApp::setup(){
    mSender.setup( "localhost", 12348 );        // we can also use 127.0.0.1, which is the same as 'localhost'
    mReceiver.setup( 12340 );
}
...
void testApp::checkOsc() {
    cout<<mReceiver.hasWaitingMessages()<<endl;
...
void testApp::keyPressed(int key){
    ofxOscMessage mVel;
    mVel.setAddress("/vel");
    mVel.addFloatArg( ofRandom(-30,30) );
    mVel.addFloatArg( ofRandom(-30,30) );
