...
// Set all the forces in the field based on a circle
void FlowField::setSin() {
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set( ofVec2f( cos(y), sin(x) ) );
        }
    }
}


// Set all the forces in the field to a corner
void FlowField::setCorner(bool X, bool Y) {
    float mX, mY;
    if(X){
        mX = 1.0;
    } else {
    ...
    for( int y=0; y<flowList.size(); y++){
        for( int x=0; x<flowList[y].size(); x++){
            flowList[y][x].set( ofVec2f( y*mY, x*mX ) );
        }
    }
}
