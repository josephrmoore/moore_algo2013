    ...
    if(e.getName() == "Speed"){
        ...
        if(colorToggle->getValue() == 1){
            ...
            for (int i = 0; i < particleList.size(); i++){
                ...
                float circVal = cos(ofGetElapsedTimeMillis())*TWO_PI;
...
void testApp::update(){
	...
	for (int i=0; i < particleList.size(); i++){
		...
		particleList[i].update(ofVec2f(sin(ofGetElapsedTimeMillis()),0 ));
	}
}
...
void testApp::addParticle(float radius) {
    ...
    float vx = sin(ofGetElapsedTimeMillis())*radius;
    float vy = cos(ofGetElapsedTimeMillis())*radius;
    float factor = 1;
